...
> 🛡️ **כבר יש לכם שירותים על השרת? אתם מוגנים.** שלבי ההקשחה של המארח (איפוס firewall, שינוי
> sshd, dist-upgrade) רצים **רק על מכונה ריקה לגמרי**. אם המתקין מזהה `ufw` פעיל, SSH על פורט
> לא-22, או שירות מאזין אחר — הוא עובר ל**מצב בטוח (safe)** ולא נוגע באף אחד מהם. ה-Hub מאזין
> רק ל-loopback ומגיע מהאינטרנט דרך ה-tunnel היוצא, אז הוא לא צריך פורטים פתוחים. אפשר לכפות
> ידנית עם `WA_HUB_HARDEN=full` או `WA_HUB_HARDEN=safe`.
